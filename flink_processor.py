import os
from pyflink.common import Types, Row, Duration, RestartStrategies
...
class EMAProcessFunction(ProcessWindowFunction[ Row,Tuple[str, str, str, Optional[float], Optional[float], str, Optional[int], Optional[float]],str,TimeWindow]):
    def open(self, runtime_context):
        # Define ValueStateDescriptors for EMA state storage
        self.previous_ema_38_state = runtime_context.get_state(ValueStateDescriptor("previous_ema_38", Types.FLOAT()))
        self.previous_ema_100_state = runtime_context.get_state(ValueStateDescriptor("previous_ema_100", Types.FLOAT()))

    def process(self, key: str, context: ProcessWindowFunction.Context[TimeWindow], elements: Iterable[Row]) -> Iterable[Tuple[str, str, str, Optional[float], Optional[float], str, Optional[int], Optional[float]]]:
